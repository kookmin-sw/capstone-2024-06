...
const Posts = () => {
  ...
  const chunkedPosts = chunkArray(PostDummyData, 3);
  ...
  return (
    <main className="flex-col border w-full h-auto justify-center items-center mt-5">
      {chunkedPosts.map((row: any[], rowIndex: number) => (
        <div key={rowIndex} className="flex justify-start space-x-20 mb-5">
          {row.map((post) => (
            <div
              key={post.PostId}
              className="flex flex-col border cursor-pointer w-auto h-auto"
            >
              <MyProfile />
              <Image
                src={post.PostImage}
              ...
              />
              <div className="flex mt-1 items-center justify-center font-bold text-sm">
                {post.PostTitle}
              </div>

              <div className="mr-2 flex justify-center items-center w-full">
                {!Liked && (
                  <button onClick={LikedBtClick} className="w-1/3 pl-5">
                    <svg
                ...
                <div className="flex justify-center items-center w-1/3">
                  ...
                  />
                  <div className="ml-2 text-xs">{12}</div>
                </div>
                <div className="flex justify-center items-center w-1/3">
                  <Image
                    src="/views.png"
